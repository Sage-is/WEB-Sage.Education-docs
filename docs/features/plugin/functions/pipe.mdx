...
Welcome to this guide on creating **Pipes** in Sage.is AI-UI! Think of Pipes as a way to **adding** a new model to Sage.is AI-UI. In this document, we'll break down what a Pipe is, how it works, and how you can create your own to add custom logic and processing to your Sage.is AI-UI models. We'll use clear metaphors and go through every detail to ensure you have a comprehensive understanding.


## Introduction to Pipes

Imagine Sage.is AI-UI as a **plumbing system** where data flows through pipes and valves. In this analogy:
...
By creating a Pipe, you're essentially crafting a custom model with the specific behavior you want, all within the Sage.is AI-UI framework.
...
- **Purpose**: Acts as the blueprint for your plugin, determining how it behaves within Sage.is AI-UI.
...
What if you want your Pipe to create **multiple models** within Sage.is AI-UI? You can achieve this by defining a `pipes` function or variable inside your `Pipe` class. This setup, informally called a **manifold**, allows your Pipe to represent multiple models.
...
- **`pipes` Function**:
  ...
  - These models will show up individually in the Sage.is AI-UI model selector.
...
Let's dive into a practical example where we'll create a Pipe that proxies requests to the OpenAI API. This Pipe will fetch available models from OpenAI and allow users to interact with them through Sage.is AI-UI.
...
- **`NAME_PREFIX`**:
  - Adds a prefix to the model names displayed in Sage.is AI-UI.
  - Default: `"OPENAI/"`.
- **`OPENAI_API_BASE_URL`**:
...
- **Purpose**: Fetches available OpenAI models and makes them accessible in Sage.is AI-UI.
...
## Using Internal Sage.is AI-UI Functions

Sometimes, you may want to leverage the internal functions of Sage.is AI-UI within your Pipe. You can import these functions directly from the `open_webui` package. Keep in mind that while unlikely, internal functions may change for optimization purposes, so always refer to the latest documentation.

Here's how you can use internal Sage.is AI-UI functions:
...
- **Function Signatures**: Refer to the latest Sage.is AI-UI codebase or documentation for the most accurate function signatures and parameters.
...
### Q1: Why should I use Pipes in Sage.is AI-UI?

**A**: Pipes allow you to add new "model" with custom logic and processing to Sage.is AI-UI. It's a flexible plugin system that lets you integrate external APIs, customize model behaviors, and create innovative features without altering the core codebase.
...
- **`pipes` Function**: Allows your Pipe to represent multiple models by returning a list of model definitions. Each model will appear individually in Sage.is AI-UI.
...
**A**: Test your Pipe by running Sage.is AI-UI in a development environment and selecting your custom model from the interface. Validate that your Pipe behaves as expected with various inputs and configurations.
...
### Q10: Where can I find the latest Sage.is AI-UI documentation?

**A**: Visit the official Sage.is AI-UI repository or documentation site for the most up-to-date information, including function signatures, examples, and migration guides if any changes occur.

---

## Conclusion

By now, you should have a thorough understanding of how to create and use Pipes in Sage.is AI-UI. Pipes offer a powerful way to extend and customize the capabilities of Sage.is AI-UI to suit your specific needs. Whether you're integrating external APIs, adding new models, or injecting complex logic, Pipes provide the flexibility to make it happen.
...
Happy coding, and enjoy extending your Sage.is AI-UI with Pipes!
