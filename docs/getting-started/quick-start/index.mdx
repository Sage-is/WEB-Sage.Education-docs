...
- **Volume mapping (`-v sage-open-webui:/app/backend/data`)** keeps your data between restarts.
...
Replace `sage-open-webui` if you named your container differently.
...
Deploying with Swarm uses a stack file to run three services (ChromaDB, Ollama, and Sage WebUI).
...
Run Sage WebUI, setting `DATA_DIR` to keep your data:
...
Expose the service with port forwarding or your ingress controller to reach Sage WebUI.
