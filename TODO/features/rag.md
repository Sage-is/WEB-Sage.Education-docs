...
For web content integration, start a query in a chat with `#`, followed by the target URL. Click on the formatted URL in the box that appears above the chat box. Once selected, a document icon appears above `Send a message`, indicating successful retrieval. Sage.is AI-UI fetches and parses information from the URL if it can.
...
A variety of parsers extract content from local and remote documents. For more, see the [`get_loader`](https://github.com/Sage-is/AI-UI/blob/2fa94956f4e500bf5c42263124c758d8613ee05e/backend/apps/rag/main.py#L328) function.
...
1. Create an OAuth 2.0 client and configure both the Authorized JavaScript origins & Authorized redirect URI to be the URL (include the port if any) you use to access your sage-is-ai-ui instance.
...
1. Create an API key and under Application restrictions and choose Websites. Then add your sage-is-ai-ui instance's URL, same as the Authorized JavaScript origins and Authorized redirect URIs settings in the step 1.
...
1. Set up the `GOOGLE_REDIRECT_URI` to my sage-is-ai-ui instance's URL (include the port, if any).
1. Then relaunch your sage-is-ai-ui instance with those three environment variables.
