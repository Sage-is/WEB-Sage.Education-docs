...
When the `Sage.is AI-UI_AUTH_TRUSTED_EMAIL_HEADER` environment variable is set, Sage.is AI-UI will use the value of the header specified as the email address of the user, handling automatic registration and login.

For example, setting `Sage.is AI-UI_AUTH_TRUSTED_EMAIL_HEADER=X-User-Email` and passing a HTTP header of `X-User-Email: example@example.com` would authenticate the request with the email `example@example.com`.

Optionally, you can also define the `Sage.is AI-UI_AUTH_TRUSTED_NAME_HEADER` to determine the name of any user being created using trusted headers. This has no effect if the user already exists.
...
services:
  sage-is-ai-ui:
    ...
    environment:
      - HOST=127.0.0.1
      - Sage.is AI-UI_AUTH_TRUSTED_EMAIL_HEADER=Tailscale-User-Login
      - Sage.is AI-UI_AUTH_TRUSTED_NAME_HEADER=Tailscale-User-Name
...
services:
  sage-is-ai-ui:
    ...
    environment:
      - HOST=127.0.0.1
      - Sage.is AI-UI_AUTH_TRUSTED_EMAIL_HEADER=Cf-Access-Authenticated-User-Email
    restart: unless-stopped
  cloudflared:
...
services:
  sage-is-ai-ui:
    ...
    environment:
      - 'HOST=127.0.0.1'
      - 'Sage.is AI-UI_AUTH_TRUSTED_EMAIL_HEADER=X-Forwarded-Email'
      - 'Sage.is AI-UI_AUTH_TRUSTED_NAME_HEADER=X-Forwarded-User'
    restart: unless-stopped
  oauth2-proxy:
