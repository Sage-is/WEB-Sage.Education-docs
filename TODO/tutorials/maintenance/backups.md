...
:::
 ...
 If you're self-hosting Sage.is AI-UI, then you may wish to institute some kind of formal backup plan in order to ensure that you retain a second and third copy of parts of your configuration.

 This guide is intended to recommend some basic recommendations for how users might go about doing that. 

 This guide assumes that the user has installed Sage.is AI-UI via Docker (or intends to do so)
...
If you're using the Docker Compose from the project repository, you will be deploying Sage Web UI using Docker volumes. 

For Ollama and Sage.is AI-UI the mounts are:
...
Some users deploy Sage Web UI with direct (fixed) binds to the host filesystem, like this:
...
services:
  ...
  sage-is-ai-ui:
    ...
    image: ghcr.io/sage-is-ai-ui/sage-is-ai-ui:${Sage.is AI-UI_DOCKER_TAG-main}
    volumes:
...
| `Sage.is AI-UI.db` | SQLite database for persistent storage of other instance data |

# File Level Backup Approaches

The first way to back up the application data is to take a file level backup approach ensuring that the persistent Sage Web UI data is properly backed up.
...
B2_BUCKET="b2://Sage.is AI-UI-backups" # Your Backblaze B2 bucket
...
SOURCE_Sage.is AI-UI_DB="$SOURCE_DIR/Sage.is AI-UI.db"
...
rclone_sync "$SOURCE_Sage.is AI-UI_DB" "$DEST_MAIN_DB"
...
# Sage.is AI-UIweeklies, maintaining 3 weekly snapshots.
...
B2_REMOTE="Sage.is AI-UIweeklies"
...
SQLITE_DB_FILE="$SOURCE/Sage.is AI-UI.db" # Path to the SQLite database file
SQLITE_BACKUP_FILE="$SOURCE/Sage.is AI-UI.db.backup" # Temporary file for SQLite backup
...
Your Sage.is AI-UI instance might be provisioned on a host (physical or virtualised) which you control. 
