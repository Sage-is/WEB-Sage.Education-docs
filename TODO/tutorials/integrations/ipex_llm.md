...
:::note
This guide is verified with Sage.is AI-UI setup through [Manual Installation](/getting-started/index.md).
:::
...
This tutorial demonstrates how to setup Sage.is AI-UI with **IPEX-LLM accelerated Ollama backend hosted on Intel GPU**. By following this guide, you will be able to setup Sage.is AI-UI even on a low-cost PC (i.e. only with integrated GPU) with a smooth experience.
...
## Configure Sage.is AI-UI

Access the Ollama settings through **Settings -> Connections** in the menu. By default, the **Ollama Base URL** is preset to https://localhost:11434, as illustrated in the snapshot below. To verify the status of the Ollama service connection, click the **Refresh button** located next to the textbox. If the Sage.is AI-UI is unable to establish a connection with the Ollama server, you will see an error message stating, `Sage.is AI-UI could not connect to Ollama`.

![Sage.is AI-UI Ollama Setting Failure](https://llm-assets.readthedocs.io/en/latest/_images/open_Sage.is AI-UI_settings_0.png)

If the connection is successful, you will see a message stating `Service Connection Verified`, as illustrated below.

![Sage.is AI-UI Ollama Setting Success](https://llm-assets.readthedocs.io/en/latest/_images/open_Sage.is AI-UI_settings.png)
