...
This page is up to date with Sage.is AI-UI release version [v0.5.1](https://github.com/Sage-is/AI-UI/releases/tag/v0.5.1), but is still a work in progress to later include more accurate descriptions, listing out options available for environment variables, defaults, and improving descriptions.
...
The following environment variables are used by `backend/open_Sage.is AI-UI/config.py` to provide Sage.is AI-UI startup
...
- Type: `str`
- Description: Sets `Sage.is AI-UI_NAME` but polls **api.sage.is** for metadata.

#### `Sage.is AI-UI_NAME`

- Type: `str`
- Default: `Sage.is AI-UI`
- Description: Sets the main Sage.is AI-UI name. Appends `(Sage.is AI-UI)` if overridden.
...
```
Sage.is AI-UI_BANNERS="[{\"id\": \"1\", \"type\": \"warning\", \"title\": \"Your messages are stored.\", \"content\": \"Your messages are stored and may be reviewed by human people. LLM's are prone to hallucinations, check sources.\", \"dismissible\": true, \"timestamp\": 1000}]"
```
...
If enabled, the following headers are forwarded:
  - `X-Sage.is AI-UI-User-Name`
  - `X-Sage.is AI-UI-User-Id`
  - `X-Sage.is AI-UI-User-Email`
  - `X-Sage.is AI-UI-User-Role`
...
#### `Sage.is AI-UI_SESSION_COOKIE_SAME_SITE`
...
When `ENABLE_OAUTH_SIGNUP` is enabled, setting `Sage.is AI-UI_SESSION_COOKIE_SAME_SITE` to `strict` can cause login failures. This is because Sage.is AI-UI uses a session cookie to validate the callback from the OAuth provider, which helps prevent CSRF attacks.
...
If the value is not set, `Sage.is AI-UI_SESSION_COOKIE_SAME_SITE` will be used as a fallback.

:::

#### `Sage.is AI-UI_AUTH_COOKIE_SECURE`
...
If the value is not set, `Sage.is AI-UI_SESSION_COOKIE_SECURE` will be used as a fallback.
...
directly. Ensure that no users are present in the database, if you intend to turn off `Sage.is AI-UI_AUTH`.

:::

#### `Sage.is AI-UI_SECRET_KEY`
...
- Example: `Authorization=Bearer heuhagfuahefj,User-Agent=Sage.is AI-UI`.
...
- Type: `str`
- Default: `sqlite:///${DATA_DIR}/Sage.is AI-UI.db`
