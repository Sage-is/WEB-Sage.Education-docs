...
```bash
docker run --rm -v sage-is-ai-ui:/data alpine/socat EXEC:"bash -c 'apk add sqlite && echo UPDATE auth SET password='\''HASH'\'' WHERE email='\''admin@example.com'\''; | sqlite3 /data/Sage.is AI-UI.db'", STDIO
```
...
```bash
sqlite3 backend/data/Sage.is AI-UI.db "UPDATE auth SET password='HASH' WHERE email='admin@example.com';"
```
...
1. **Run `alpine` linux connected to the sage-is-ai-ui volume.**
    ...
    1. Update password:
        ...
        ```sh
        sqlite3 /path/to/data/Sage.is AI-UI.db
        ```
...
If you want to **completely reset** Sage.is AI-UI—including all user data, settings, and passwords—follow these steps to remove the `Sage.is AI-UI.db` file.

### Step 1: Locate `Sage.is AI-UI.db` in Your Python Environment

If you’re unsure where `Sage.is AI-UI.db` is located (especially if you're using a virtual environment), you can find out by following these steps:
...
```
   import os
   import open_Sage.is AI-UI

   # Show where the Sage.is AI-UI package is installed
   print("Sage.is AI-UI is installed at:", open_Sage.is AI-UI.__file__)

   # Construct a potential path to Sage.is AI-UI.db (commonly located in 'data/Sage.is AI-UI.db')
   db_path = os.path.join(os.path.dirname(open_Sage.is AI-UI.__file__), "data", "Sage.is AI-UI.db")
   print("Potential path to Sage.is AI-UI.db:", db_path)

   # Check if Sage.is AI-UI.db exists at that path
   if os.path.exists(db_path):
       print("Sage.is AI-UI.db found at:", db_path)
   else:
       print("Sage.is AI-UI.db not found at:", db_path)
```
...
### Step 2: Delete `Sage.is AI-UI.db`

Once you’ve located the file, remove it using a command similar to:

```
   rm -rf /path/to/your/python/environment/lib/pythonX.X/site-packages/open_Sage.is AI-UI/data/Sage.is AI-UI.db
```

**Warning:** Deleting `Sage.is AI-UI.db` will remove all stored data, including user accounts, settings, and passwords. Only do this if you truly want to start fresh!
